...
function numToInt(num){
  for(var i=0;i<num.length;i++){
    if(num[i]=="."){
      return [num*Math.pow(10,num.length-i-1),Math.pow(10,num.length-i-1)]
    }
  }
  return [num,1];/*數字,放大縮小倍率*/
}

function numToFloat(num,rate){
  if(rate>0){
    return num / rate;
  }else{
    return num
...
function add(numA,numB){
  ...
  if(a[1]>b[1]){
    return numToFloat(parseFloat(a[0])+parseFloat(numB)*a[1],a[1])
  }else{
    return numToFloat(parseFloat(numA)*b[1]+parseFloat(b[0]),b[1])
  }
}
...
function mul(numA,numB){
  var a = numToInt(numA),b = numToInt(numB);
  if(a[1]>b[1]){
    return numToFloat(parseFloat(a[0])*parseFloat(numB)*a[1],a[1]*a[1])
  }else{
    return numToFloat(parseFloat(numA)*b[1]*parseFloat(b[0]),b[1]*b[1])
  }
}
...
*/
function calNum(arr){
  var temp = new Array(), arrTemp = new Array(),i;
...
function btnClickInput(){
  var _showMain = "",recordShow = "",_calArray = new Array();
  
  /*滑鼠點擊版*/
  document.getElementById("btn0").onclick = function(){
      if(_showMain[0] != 0 || (_showMain[0] ==0 && _showMain[1] == ".")){
        /*避免開頭連按0*/
...
/*-----↑↑↑↑↑↑↑---加減乘除括號 按鍵額外處裡--↑↑↑↑↑↑↑---*/
  ...
  document.onkeydown = function(e){
    var i;
    if(((e.which ==96 || e.keyCode ==96) && _showMain[0] != 0) || (_showMain[0] == 0 && _showMain[1] == "." && (e.which ==96 || e.keyCode ==96))){
      if(_calArray.length == 1 && _calArray[0] !="("){
          _calArray = [];
          document.getElementById("recordShow").innerHTML = _calArray;
       }
